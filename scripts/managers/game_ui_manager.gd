extends Node

signal ui_state_changed(state_name: String)

enum Screen {
	GAME,
	IDLE,
	UPGRADES,
	STATS
}

var current_screen: Screen = Screen.GAME
var current_ui_state := "gameplay"
var showing_upgrades := false

@onready var game_board := $"../GameMargins/GameBoard"
@onready var idle_panel := $"../IdleMargins/GeneratorPanel"
@onready var upgrades_margins := $"../UpgradesMargins"
@onready var upgrades_panel := $"../UpgradesMargins/UpgradesPanel"
@onready var game_margins := $"../GameMargins"
@onready var idle_margins := $"../IdleMargins"
@onready var debug_margins := $"../DebugMargins"  # Add this line
@onready var debug_panel := $"../DebugMargins/DebugPanel"  # Add this line
@onready var to_idle_button := $"../GameMargins/GameBoard/TopBarPanel/GameOverBox/Buttons/IdleScreenSwap"
@onready var play_again_button := $"../GameMargins/GameBoard/TopBarPanel/GameOverBox/Buttons/Restart"
@onready var convert_and_stash := $"../GameMargins/GameBoard/TopBarPanel/GameOverBox/Buttons/ConvertAndStash"
@onready var top_bar_panel := $"../GameMargins/GameBoard/TopBarPanel"
@onready var score_label := $"../GameMargins/GameBoard/TopBarPanel/TopBar/ScoreValue"
@onready var board_panel := $"../GameMargins/GameBoard/BoardPanel"
@onready var queue_panel := $"../GameMargins/GameBoard/QueuePanel"
@onready var board_canvas := $"../GameMargins/GameBoard/BoardPanel/BoardContainer/BoardCanvas"
@onready var stats_margins := $"../StatsMargins"
@onready var stats_panel := $"../StatsMargins/StatsPanel"
@onready var current_score: int = 0

func _ready():
	show_game_screen()
	# Connect UI button signals
	to_idle_button.pressed.connect(show_idle_screen)
	play_again_button.pressed.connect(_on_play_again_pressed)
	convert_and_stash.pressed.connect(_on_convert_stash_pressed)
	
	# Connect game board signals
	if not board_canvas.is_connected("game_over", _on_board_canvas_game_over):
		board_canvas.game_over.connect(_on_board_canvas_game_over)
	board_canvas.score_changed.connect(_on_current_score_changed)
	
	# NEW: Connect to StatsTracker signals instead of calling methods directly
	if StatsTracker.has_signal("games_converted"):
		StatsTracker.connect("games_converted", _on_games_converted)
	
	# Debug setup
	if debug_panel:
		debug_panel.visible = false

func _on_convert_stash_pressed():
	# Request conversion instead of doing it ourselves
	StatsTracker.request_conversion_of_pending_games()
	
	# Disable button immediately to prevent double-clicks
	convert_and_stash.disabled = true
	
	# Show "Converting..." feedback
	top_bar_panel.get_node("GameOverBox/Label").text = "Converting games..."
# NEW: Handle conversion completion via signal
func _on_games_converted(total_earned: float, games_count: int):
	# Update UI to show conversion results
	top_bar_panel.get_node("GameOverBox/Label").text = "Conversion Complete!"
	top_bar_panel.get_node("GameOverBox/Scores/MovesLabel").text = "Games Converted: %d" % games_count
	top_bar_panel.get_node("GameOverBox/Scores/ScoreLabel").text = "Currency Earned: %s" % CurrencyManager.format_currency(total_earned)
	
	print("UI: Conversion complete. %d games converted for %s currency" % [games_count, CurrencyManager.format_currency(total_earned)])

func _on_board_canvas_game_over(stats: GameStats) -> void:
	# Just show the game over UI - don't handle game logic here
	show_game_over(stats)

func _on_current_score_changed(value: int):
	current_score = value
	score_label.text = str(current_score)

func _on_play_again_pressed():
	reset_to_gameplay()
	board_canvas.reset_board()

# IMPROVED: Cleaner separation of UI and game logic
func show_game_over(stats: GameStats):
	current_ui_state = "game_over"
	emit_signal("ui_state_changed", current_ui_state)
	
	# Enable conversion button
	convert_and_stash.disabled = false
	
	# Update game over display
	top_bar_panel.get_node("GameOverBox").visible = true
	top_bar_panel.get_node("GameOverBox/Scores/ScoreLabel").text = "Score: %d" % stats.score
	top_bar_panel.get_node("GameOverBox/Scores/MovesLabel").text = "Moves: %d" % stats.moves
	
	# Let StatsTracker handle the game recording (it will emit signals for other systems)
	StatsTracker.record_game(stats)
	
	# Show how many games are pending conversion
	var pending_count = StatsTracker.get_pending_games_count()  # You'll need to add this method
	top_bar_panel.get_node("GameOverBox/Label").text = "Game Over! (%d games ready to convert)" % pending_count
	
	# UI animations and layout
	_animate_game_over_ui()

# EXTRACTED: Separate UI animation logic
func _animate_game_over_ui():
	var tween := get_tree().create_tween()
	tween.tween_property(top_bar_panel, "custom_minimum_size:y", 300, 0.3).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
	
	# Disable board interaction
	board_panel.mouse_filter = Control.MOUSE_FILTER_IGNORE
	
	# Adjust layout
	queue_panel.visible = false
	top_bar_panel.get_node("TopBar").visible = false
	top_bar_panel.set_stretch_ratio(2)
	board_panel.set_stretch_ratio(1.25)

func reset_to_gameplay():
	current_ui_state = "gameplay"
	emit_signal("ui_state_changed", current_ui_state)
	
	# Re-enable board interaction
	board_panel.mouse_filter = Control.MOUSE_FILTER_STOP
	
	# UI animations and layout
	_animate_return_to_game()

# EXTRACTED: Separate UI animation logic
func _animate_return_to_game():
	var tween := get_tree().create_tween()
	tween.tween_property(top_bar_panel, "custom_minimum_size:y", 0, 0.2)
	
	# Reset layout
	top_bar_panel.get_node("GameOverBox").visible = false
	top_bar_panel.get_node("TopBar").visible = true
	top_bar_panel.set_stretch_ratio(1)
	board_panel.set_stretch_ratio(4)
	queue_panel.visible = true

# Input handling
func _input(event: InputEvent):
	if event.is_action_pressed("show_board"):
		current_screen = Screen.GAME
		swap_screen()
	if event.is_action_pressed("show_upgrades"):
		current_screen = Screen.UPGRADES
		swap_screen()
	if event.is_action_pressed("show_gens"):
		current_screen = Screen.IDLE
		swap_screen()
	if event.is_action_pressed("show_stats"):
		current_screen = Screen.STATS
		swap_screen()
	if event.is_action_pressed("toggle_debug"):
		toggle_debug_panel()

# Debug panel management
func toggle_debug_panel():
	if debug_panel:
		debug_panel.visible = !debug_panel.visible
		debug_margins.visible = debug_panel.visible
		
		if debug_panel.visible:
			debug_margins.move_to_front()

# Screen management methods (unchanged)
func shrink_board_panel():
	game_board.visible = false
	game_margins.visible = false

func slide_board_panel():
	game_board.visible = true
	game_margins.visible = true

func shrink_idle_panel():
	idle_margins.visible = false
	idle_panel.visible = false

func slide_idle_panel():
	idle_margins.visible = true
	idle_panel.visible = true

func shrink_stats_panel():
	stats_panel.visible = false
	stats_margins.visible = false

func slide_stats_panel():
	stats_panel.visible = true
	stats_margins.visible = true

func shrink_upgrades_panel():
	upgrades_margins.visible = false
	upgrades_panel.visible = false

func slide_upgrades_panel():
	upgrades_panel.refresh_panel()
	upgrades_margins.visible = true
	upgrades_panel.visible = true

func show_idle_screen():
	shrink_board_panel()
	shrink_stats_panel()
	shrink_upgrades_panel()
	slide_idle_panel()

func show_game_screen():
	shrink_stats_panel()
	shrink_idle_panel()
	shrink_upgrades_panel()
	slide_board_panel()

func show_stats_screen():
	shrink_board_panel()
	shrink_idle_panel()
	shrink_upgrades_panel()
	slide_stats_panel()

func show_upgrades_screen():
	shrink_board_panel()
	shrink_idle_panel()
	shrink_stats_panel()
	slide_upgrades_panel()

func swap_screen():
	if current_screen == Screen.GAME:
		show_game_screen()
	if current_screen == Screen.IDLE:
		show_idle_screen()
	if current_screen == Screen.STATS:
		show_stats_screen()
	if current_screen == Screen.UPGRADES:
		show_upgrades_screen()

func transition_to(screen_name: String):
	current_ui_state = screen_name
	emit_signal("ui_state_changed", current_ui_state)
